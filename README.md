**Md. Khalid Hasan**
Dhaka, Bangladesh  

**About Me**

I am a GIS and Environmental Research enthusiast with hands-on experience in GIS digitization, spatial analysis, and remote sensing. I work with satellite and drone imagery for land use mapping, environmental analysis, and spatial decision support. My interests focus on applying GIS, Remote Sensing, and AI-based approaches to solve real-world environmental and urban problems.

**Research Interests**

- Flood risk assessment and river basin analysis using GIS and Remote Sensing  
- AI-based detection of roads, buildings, and farmland from high-resolution imagery  
- Automated Land Use / Land Cover (LULC) classification and urban growth analysis  
- Urban Heat Island (UHI) and green space analysis  
- Integration of Optical and SAR data for environmental planning  

**Education**

BSc (Honours) in Geography and Environment  
Dhaka College, University of Dhaka  
2022 – Present  
- Current CGPA: 3.47 / 4.00  
- Expected Result: First Class Honours  

**Professional Experienc**e
Center for Environmental and Geographic Information Services (CEGIS)  
Research Consultant – Remote Sensing Division
Apr 2025 – Jun 2025  

- Land use mapping using high-resolution satellite imagery  
- GIS digitization, visual interpretation, and thematic extraction  
- Shapefile preparation with topology rules  

Geomatics and Geoinformatics Solutions Ltd. (GMGI)  
GIS Analyst  
Nov 2024 – Apr 2025  
- Spatial data quality control and topology validation  
- Feature extraction from satellite imagery  
- Attribute management and GIS data processing  

Geo Planning for Advanced Development (GPAD)  
**GIS Analyst (22 Roads Project)
- Road digitization from drone imagery  
- Buffer analysis and topology validation  

**Technical & Research Skills**

- GIS & Remote Sensing: ArcGIS Pro, ArcMap, digitization, buffering  
- Spatial Analysis: LULC mapping, thematic analysis, decision support  
- Data Analysis: R (basic), SPSS  
- Research Skills: literature review, methodology design, reporting  

**Training & Certifications**

- Professional GIS & Remote Sensing  
- Research Paper Writing – Grameenphone Academy  
- SPSS & R Programming (Ongoing)

**Current Goal**

Preparing for fully funded Master’s programs in GIS, Remote Sensing, Environmental Science, and Spatial Data Science.
